...
export class RepMapRegionSet<L> implements RegionSet<L> {
     ...
     */
    private intersect(rectangle1: Rect, rectangle2: Rect): boolean {
     ...
     */
    private connected(rectangle1: Rect, rectangle2: Rect):boolean {
    ...
    private checkRep():void {
        ...
        for (const [label, rectangleArray] of this.map) {
            for (const rectangle1 of rectangleArray) {
                ...
                if (!contiguous) {throw new Error("Region of the same label must be contiguous");}
            }
        }
    ...
    public bounds(label: L): Rect | undefined {
        if (!this.map.has(label)){return undefined;}
        else {
            const region = this.map.get(label); // region is the array of rects
...
export class RepArrayRegionSet<L> implements RegionSet<L> {
    ...
    private checkRep():void {
        ...
        for (const label of labelSet){
            const labeledRegion = new Set<Rect>; // set of rectangles with the same current label
            for (let i = 0; i < arrayLen; i++){
    ...
    public add(label: L, rect: Rect): void {
        ...
        const arrayLen = this.gridSize * this.gridSize;
        const labeledRegion = new Set<Rect>; // set of rectangles with the current label
        for (let i = 0; i < arrayLen; i++){
            if (this.array[i] === label){
                const thisRect = this.getRectangle(i);
                labeledRegion.add(thisRect);
