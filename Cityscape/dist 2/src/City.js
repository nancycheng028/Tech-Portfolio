...
const RegionSet_1 = require("./RegionSet");
/**
...
class City {
    ...
    constructor(gridSize) {
        ...
        for (let i = 0; i < this.gridSize; i++) {
            const newRegionSet = (0, RegionSet_1.makeRegionSet)(this.gridSize);
    ...
    getRegionSetArray(floorNum) {
        ...
        for (let i = 0; i < floorArrayLength; i++) {
            ...
            if (cellLabel?.size !== 0) {
                (0, assert_1.default)(cellLabel?.size === 1); //cellLabel is a set of size 1
    ...
    checkRep() {
        ...
        for (let i = 0; i < this.gridSize; i++) {
            ...
            const regionRect = new Rect_1.Rect(0, 0, this.gridSize, this.gridSize);
            const regionOwners = this.cityArray[i]?.owners(regionRect);
            (0, assert_1.default)(regionOwners !== undefined);
        ...
        for (const label of allLabelSet) {
            ...
            const indexofLabelInBuilding = new Array();
            ...
            for (let i = 0; i < indexofLabelInBuilding.length - 1; i++) {
                ...
                (0, assert_1.default)(thisIndexSet !== undefined);
                (0, assert_1.default)(nextIndexSet !== undefined);
    ...
    expand(label, floor, rect) {
        ...
        if (floor !== 0) {
            ...
            (0, assert_1.default)(belowFloorIndex !== undefined);
            // console.log(belowFloorIndex)
            const curFloorOwners = this.cityArray[floor]?.owners(new Rect_1.Rect(0, 0, this.gridSize, this.gridSize));
            if (curFloorOwners !== undefined) {
                (0, assert_1.default)(curFloorOwners !== undefined);
                // console.log('1')
    ...
    bounds(label) {
        ...
        (0, assert_1.default)(firstBound !== undefined);
        ...
        return { footprint: new Rect_1.Rect(minx1, miny1, maxx2, maxy2), height: maxHeight + 1 };
    }
    toString() {
        let result = '';
        for (let i = 0; i < this.gridSize; i++) {
            ...
            if (thisRegion != undefined) {
                (0, assert_1.default)(thisRegion !== undefined);
                result += `Floor ${i}:\n${thisRegion.toString()}\n`;
            }
        }
        return result;
