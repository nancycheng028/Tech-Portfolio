        ...
        if path in self.functions:
            try:
                ...
                if not "application/json" in content_type.lower():
                    raise ValueError(
                        "PUSH data doesn't look like json. Needs application/json content type."
                    )
            ...
            self.send_error(
                ...
                "function not found: "
                + path
                + " , while registered functions are: "
                + str(self.functions),
            )
        return
...
# returns None
RPCServerHandler.register_function(
    lambda d: RPCServerHandler.reload_modules(), "restart"
)
